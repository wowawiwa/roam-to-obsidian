// Replace #toto and #[[toto]] by [[toto]]
//
// Execute just like that for dry run.
//
// Then:
// executeTagConversion()

(function setupTagConverter() {
  const transformText = (text) => {
    ...
    return parts
      .map((part) => {
        ...
        // 1. Multi-word tags: #[[foo bar]] -> [[foo bar]]
        let updated = part.replace(/(^|\s)#(\[\[.*?\]\])/g, "$1$2");

        // 2. Single-word tags: #foo -> [[foo]] (skipping hex colors)
        updated = updated.replace(/(^|\s)#([\w\/\-]+)/g, (match, space, tag) => {
          const isHex = /^(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(tag);
          if (isHex) return match;
          return `${space}[[${tag}]]`;
        });
  ...
  window.executeTagConversion = async function () {
    console.log(`Starting safe execution for ${pendingUpdates.length} blocks...`);
    let successCount = 0;
      ...
      while (!written && retries < 5) {
        try {
          ...
          if (err.message && err.message.includes("rate limit exceeded")) {
            ...
            break; // Skip unrecoverable errors
          }
        }
      }

      // Base pacing delay: ~65ms per item = ~15 requests/sec max (~900/min)
      await sleep(65);
  ...
  console.log(`%c[DRY RUN COMPLETE] Found ${pendingUpdates.length} blocks to update.`, "color: #00ffff; font-weight: bold;");
  console.log("1. Type `copy(window.dryRunData)` to copy the JSON list.");
  console.log("2. Type `executeTagConversion()` to run the update safely under rate limits.");
})();
